...
对比 V14.0.1（成功）和 V14.9（失败）版本的代码：
- **V14.0.1（成功）**: 只使用 `-t` 参数
- **V14.9（失败）**: 同时使用 `-t` 和 `-frames:v` 参数
...
**修复方案** (V14.10):
移除 `_trim_segment` 方法中的 `-frames:v` 参数（第736行）：
```python
# 修复后的代码
'-t', f"{duration:.3f}",
'-c', 'copy',
...
- `scripts/understand/render_clips.py`
  - `_trim_segment()` 方法（第 730-740 行）- 已修复
