...
export async function extractKeyframes(
  ...
  if (fps !== undefined) {
    ...
    const targetFrameCount = Math.floor(durationSeconds * fps);
    const intervalMs = Math.floor(1000 / fps);

    console.log(`📹 [关键帧提取-FPS模式] 时间范围: ${(actualStartTimeMs/1000).toFixed(1)}s-${(actualEndTimeMs/1000).toFixed(1)}s, fps: ${fps}, 目标帧数: ${targetFrameCount}, 间隔: ${(intervalMs/1000).toFixed(2)}秒`);
    ...
    return await extractFramesInternal({
      ...
      totalDuration  // 添加totalDuration参数
    });
  }
