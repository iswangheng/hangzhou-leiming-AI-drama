...
| 函数名 | 功能说明 |
|--------|----------|
| `VideoOverlayRenderer.apply_overlay()` | 应用花字叠加，可传 `project_name` 触发智能定位 |
| `_get_subtitle_region()` | 获取字幕区域（优先读缓存，否则实时检测） |
| `_calculate_text_layout()` | 根据字幕位置计算剧名/免责声明的最优坐标和布局模式 |
...
| 版本 | 日期 | 主要变更 |
|-----|------|----------|
| V18.5 | 2026-03-15 | 渲染默认启用视频压缩（CRF=23，新增--no-compress参数） |
| V18.4 | 2026-03-15 | 花字剧名/免责声明智能定位（跟随字幕区域，防压字幕） |
| V18.3 | 2026-03-15 | 角标展示脚本、force_badge_style参数、依赖补全 |
